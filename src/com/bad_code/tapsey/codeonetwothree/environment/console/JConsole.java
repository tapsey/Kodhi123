...
import com.bad_code.tapsey.codeonetwothree.environment.SystemCommands;
import com.bad_code.tapsey.codeonetwothree.environment.view.Window;


@SuppressWarnings("serial")
public class JConsole extends JScrollPane implements GUIConsoleInterface,
		Runnable, KeyListener, MouseListener, ActionListener,
	...
	// by tapsey
	static Comms c;
	static SystemCommands sysc = new SystemCommands();
	...
	public JConsole() {
		...
		c= sysc;
		clearPane();
		Window.attachjConsole(this);
	}

	private JConsole(InputStream cin, OutputStream cout) {
		super();
	...
	private void doCommandCompletion(String part) {
		...
		while (i >= 0
				&& (Character.isJavaIdentifierPart(part.charAt(i)) || part
						.charAt(i) == '.'))
			i--;
	...
	public static void registerForInput(Comms cIn) {
		c = cIn;

	}
	
	public static void deRegisterForInput() {
		c = sysc;
