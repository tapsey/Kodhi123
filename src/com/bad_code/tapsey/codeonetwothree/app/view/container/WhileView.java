...
public class WhileView extends ElementView {
	BooleanDock bd;
	UpperDockPane docksPane = new UpperDockPane(DockLocation.WHILE);
	public WhileView(Element element) {
		super(element);
		...
		xprsnlblWhile.setText("Kana");
		panel_4.add(xprsnlblWhile);
		
		JPanel conditionDockContainer = new JPanel();
		conditionDockContainer.setLayout(new BorderLayout(0, 0));
		bd = new BooleanDock(DockLocation.WHILE_CONDTION);
		conditionDockContainer.add(bd.getView(), BorderLayout.CENTER);
		panel_4.add(conditionDockContainer);
		
		ExpressionLabel xprsnlblDzokororaZvinotevera = new ExpressionLabel();
		xprsnlblDzokororaZvinotevera.setText("dzokorora zvinotevera");
		panel_4.add(xprsnlblDzokororaZvinotevera);
	...
	public Element<Boolean> getWhileConditionElement(){
